...
/*
 * Caller for the bridge_rpc example.
 *
 ...
 *
 * Usage:
 *   BridgeRpcCaller <ws-url> <token>
 *   LIVEKIT_URL=... LIVEKIT_TOKEN=... BridgeRpcCaller
 *
...
int main(int argc, char *argv[]) {
  ...
  if (url.empty() || token.empty()) {
    std::cerr << "Usage: BridgeRpcCaller <ws-url> <token>\n"
  ...
  while (g_running.load()) {
    ...
    try {
      auto response =
          bridge.performRpc("receiver", "print", message, std::nullopt);
