...
void printUsage(const char *prog) {
  std::cerr
      ...
      << "  --raw-nv12 <host:port>   TCP server for raw NV12 (default 127.0.0.1:5004)\n"
...
bool parseArgs(int argc, char *argv[], std::string &url, std::string &token,
               bool &enable_e2ee, std::string &e2ee_key, RawNv12Args &raw_nv12) {
  enable_e2ee = false;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--enable_e2ee") { enable_e2ee = true; continue; }
    if (a.rfind("--raw-nv12", 0) == 0) {
      std::string v = get_flag_value("--raw-nv12", i);
      if (v.empty()) v = "127.0.0.1:5004";
      size_t colon = v.find(':');
      if (colon != std::string::npos) {
        raw_nv12.host = v.substr(0, colon);
        try {
          raw_nv12.port = static_cast<std::uint16_t>(std::stoul(v.substr(colon + 1)));
        } catch (...) { raw_nv12.port = 5004; }
      } else {
        raw_nv12.host = v;
      }
      continue;
    ...
    if (a.rfind("--raw-width", 0) == 0) {
      ...
      if (!v.empty()) try { raw_nv12.width = std::stoi(v); } catch (...) {}
      continue;
    }
    if (a.rfind("--raw-height", 0) == 0) {
      std::string v = get_flag_value("--raw-height", i);
      if (!v.empty()) try { raw_nv12.height = std::stoi(v); } catch (...) {}
      continue;
    }
    if (a.rfind("--raw-fps", 0) == 0) {
      std::string v = get_flag_value("--raw-fps", i);
      if (!v.empty()) try { raw_nv12.fps = std::stoi(v); } catch (...) {}
      continue;
    }
    if (a.rfind("--url", 0) == 0) {
  ...
  if (url.empty()) { const char *e = std::getenv("LIVEKIT_URL"); if (e) url = e; }
  if (token.empty()) { const char *e = std::getenv("LIVEKIT_TOKEN"); if (e) token = e; }
  if (e2ee_key.empty()) { const char *e = std::getenv("LIVEKIT_E2EE_KEY"); if (e) e2ee_key = e; }
  return !(url.empty() || token.empty());
}

class LoggingDelegate : public livekit::RoomDelegate {
public:
  void onParticipantConnected(livekit::Room &, const livekit::ParticipantConnectedEvent &ev) override {
    std::cout << "[Room] participant connected: " << ev.participant->identity() << "\n";
  }
  void onTrackSubscribed(livekit::Room &, const livekit::TrackSubscribedEvent &ev) override {
    std::cout << "[Room] track subscribed: " << (ev.publication ? ev.publication->name() : "?") << "\n";
  }
};
...
int main(int argc, char *argv[]) {
  ...
  if (enable_e2ee) {
    ...
    if (!e2ee_key.empty()) enc.key_provider_options.shared_key = toBytes(e2ee_key);
  ...
  const std::size_t expected_size = static_cast<std::size_t>(width) * static_cast<std::size_t>(height) * 3 / 2;

  auto videoSource = std::make_shared<VideoSource>(width, height);
  auto videoTrack = LocalVideoTrack::createLocalVideoTrack("nv12_tcp", videoSource);
  ...
  auto rawSource = std::make_unique<publish_nv12::RawNv12TcpSource>(
      raw_nv12.host, raw_nv12.port, width, height, raw_nv12.fps,
      [videoSource, expected_size, width, height](publish_nv12::RawNv12Frame frame) {
        if (frame.data.size() != expected_size) {
          std::cerr << "Raw NV12 frame size mismatch\n";
          return;
        }
        try {
          VideoFrame vf(width, height, VideoBufferType::NV12, std::move(frame.data));
          videoSource->captureFrame(vf, frame.timestamp_us, VideoRotation::VIDEO_ROTATION_0);
        } catch (const std::exception &e) {
