...
/*
 * Caller (controller) for the SessionManager mute/unmute example.
 *
 ...
 *
 * Usage:
 *   MuteUnmuteCaller <ws-url> <token> [receiver-identity]
 *   LIVEKIT_URL=... LIVEKIT_TOKEN=... MuteUnmuteCaller [receiver-identity]
 *
...
int main(int argc, char *argv[]) {
  ...
  if (url.empty() || token.empty()) {
    std::cerr
        << "Usage: MuteUnmuteCaller <ws-url> <token> [receiver-identity]\n"
        << "   or: LIVEKIT_URL=... LIVEKIT_TOKEN=... MuteUnmuteCaller "
           "[receiver-identity]\n"
  ...
  std::thread toggle_thread([&]() {
    ...
    while (toggle_running.load()) {
      ...
      try {
        if (currently_muted) {
          sm.requestRemoteTrackUnmute(receiver_identity, "mic");
          std::cout << "[caller]   mic: unmuted OK\n";
        } else {
          sm.requestRemoteTrackMute(receiver_identity, "mic");
      ...
      try {
        if (currently_muted) {
          sm.requestRemoteTrackUnmute(receiver_identity, "cam");
          std::cout << "[caller]   cam: unmuted OK\n";
        } else {
          sm.requestRemoteTrackMute(receiver_identity, "cam");
