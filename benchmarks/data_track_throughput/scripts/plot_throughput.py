...
def plot_title(title: str, dataset_name: str) -> str:
    return f"{title} ({dataset_name})" if dataset_name else title
...
def create_expected_vs_actual_plot(
    ...
    ax.set_title(plot_title("Expected vs Actual Throughput", dataset_name))
...
def create_drops_vs_throughput_plot(
    summary_rows: Sequence[Dict[str, object]],
    output_dir: Path,
    dataset_name: str,
) -> None:
    ...
    if use_gbps:
        ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda v, _: f"{v / 1000:.1f}"))
    ax.set_title(plot_title("Dropped Messages vs Expected Throughput", dataset_name))
    unit = "Gbps" if use_gbps else "Mbps"
    ax.set_xlabel(f"Expected throughput ({unit})")
...
def build_heatmap(
    ...
    ax.set_title(plot_title(title, dataset_name))
...
def create_delivery_ratio_plot(
    summary_rows: Sequence[Dict[str, object]],
    output_dir: Path,
    dataset_name: str,
) -> None:
...
def create_actual_throughput_plot(
    summary_rows: Sequence[Dict[str, object]],
    output_dir: Path,
    dataset_name: str,
) -> None:
...
def create_message_latency_histogram(
    ...
    ax.set_title(plot_title("Message Latency Distribution", dataset_name))
...
def create_message_interarrival_plot(
    message_rows: Sequence[Dict[str, object]],
    output_dir: Path,
    dataset_name: str,
) -> None:
    ...
    ax.set_title(plot_title("Inter-arrival Time Across Received Messages", dataset_name))
...
def generate_plots(
    ...
    create_expected_vs_actual_plot(summary_rows, output_dir, dataset_name)
    create_drops_vs_throughput_plot(summary_rows, output_dir, dataset_name)
    create_actual_throughput_plot(summary_rows, output_dir, dataset_name)
    create_delivery_ratio_plot(summary_rows, output_dir, dataset_name)
    create_latency_heatmap(summary_rows, latency_stats, output_dir, dataset_name)
    create_median_latency_heatmap(summary_rows, latency_stats, output_dir, dataset_name)
    if message_rows:
        create_message_latency_histogram(message_rows, output_dir, dataset_name)
        create_message_interarrival_plot(message_rows, output_dir, dataset_name)
...
def main() -> int:
    ...
    generate_plots(summary_rows, message_rows, latency_stats, output_dir, csv_dir.name)
