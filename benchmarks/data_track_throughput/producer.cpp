...
void printUsage(const char *prog) {
  std::cout << "Usage:\n"
            ...
            << "  --messages-per-scenario <count>  Default sweep message count. Default: 30\n"
            << "  --rate-hz <hz>                   Run a single scenario instead of the full sweep\n"
            << "  --packet-size <size>             Single-scenario packet size, e.g. 1mb\n"
...
buildScenarioPlan(const ProducerOptions &options,
  ...
  if (options.single_rate_hz.has_value()) {
    ...
    request.run_id = 1;
    request.scenario_name = defaultScenarioName(*options.single_packet_size_bytes,
                                                *options.single_rate_hz);
  ...
  auto scenarios = makeDefaultScenarioPlan(
...
ProducerStats runScenario(LocalDataTrack &track,
  ...
  for (std::size_t index = 0; index < request.message_count && g_running.load();
    ...
    auto payload = makePayload(request, static_cast<std::uint64_t>(index),
                               send_time_us);
