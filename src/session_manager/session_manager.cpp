...
bool SessionManager::connect(const std::string &url, const std::string &token,
  ...
  auto delegate = std::make_unique<SessionManagerRoomDelegate>(*this);
...
void SessionManager::disconnect() {
...

  {
    ...
    if (!connected_) {
      LK_LOG_WARN(
          "Attempting to disconnect an already disconnected SessionManager. "
...
std::optional<std::string> SessionManager::performRpc(
...
void SessionManager::executeTrackAction(
