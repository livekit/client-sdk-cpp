...
/// @file managed_local_video_track.cpp
/// @brief Implementation of ManagedLocalVideoTrack.
...
ManagedLocalVideoTrack::ManagedLocalVideoTrack(
...
ManagedLocalVideoTrack::~ManagedLocalVideoTrack() { release(); }

bool ManagedLocalVideoTrack::pushFrame(const std::vector<std::uint8_t> &rgba,
...
bool ManagedLocalVideoTrack::pushFrame(const std::uint8_t *rgba,
...
void ManagedLocalVideoTrack::mute() {
  std::lock_guard<std::mutex> lock(mutex_);
  if (!released_ && track_) {
    track_->mute();
  }
...
void ManagedLocalVideoTrack::unmute() {
  std::lock_guard<std::mutex> lock(mutex_);
  if (!released_ && track_) {
    track_->unmute();
  }
...
bool ManagedLocalVideoTrack::isReleased() const noexcept {
  std::lock_guard<std::mutex> lock(mutex_);
  return released_;
}

void ManagedLocalVideoTrack::release() {
  std::lock_guard<std::mutex> lock(mutex_);
  if (released_) {
    return;
  }
  ...
  if (participant_ && publication_) {
    try {
      ...
      LK_LOG_WARN("ManagedLocalVideoTrack unpublishTrack error, continuing with "
                  "cleanup");
    }
  }
