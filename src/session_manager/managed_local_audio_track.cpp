...
/// @file managed_local_audio_track.cpp
/// @brief Implementation of ManagedLocalAudioTrack.
...
ManagedLocalAudioTrack::ManagedLocalAudioTrack(
...
ManagedLocalAudioTrack::~ManagedLocalAudioTrack() { release(); }

bool ManagedLocalAudioTrack::pushFrame(const std::vector<std::int16_t> &data,
...
bool ManagedLocalAudioTrack::pushFrame(const std::int16_t *data,
                                  int samples_per_channel, int timeout_ms) {
...
void ManagedLocalAudioTrack::mute() {
  std::lock_guard<std::mutex> lock(mutex_);
  if (!released_ && track_) {
    track_->mute();
  }
...
void ManagedLocalAudioTrack::unmute() {
  std::lock_guard<std::mutex> lock(mutex_);
  if (!released_ && track_) {
    track_->unmute();
  }
...
bool ManagedLocalAudioTrack::isReleased() const noexcept {
  std::lock_guard<std::mutex> lock(mutex_);
  return released_;
}

void ManagedLocalAudioTrack::release() {
  std::lock_guard<std::mutex> lock(mutex_);
  if (released_) {
    return;
  }
  ...
  if (participant_ && publication_) {
    try {
      ...
      LK_LOG_WARN("ManagedLocalAudioTrack unpublishTrack error, continuing with "
                  "cleanup");
    }
  }
