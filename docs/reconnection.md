...
`onDisconnected` callback. Create a new connection attempt only after the room
has reached `Disconnected`.
...
Room delegate callbacks run on the Rust-managed FFI callback thread. A callback
must not block, wait for a future, reconnect the room, or do substantial work.
Post a small event to the application's executor, event loop, or worker queue
instead:
...
After terminal `onDisconnected`, obtain fresh credentials if needed and perform
the next `disconnect`/`connect` lifecycle from an application thread.
